...
async function fetchWithRetry(url, config, maxRetries = 3) {
  for (let i = 0; i < maxRetries; i++) {
    try {
      // Ajouter validateStatus pour accepter les codes 4xx
      const finalConfig = {
        ...config,
        validateStatus: (status) => status < 500
      };
      
      // Utiliser axios directement (pas axiosInstance)
      return await axios(url, finalConfig);
      ...
      // Ne PAS retry sur 403/404 (sortir immédiatement)
      ...
      // Backoff exponentiel : 500ms, 1500ms, 4500ms
